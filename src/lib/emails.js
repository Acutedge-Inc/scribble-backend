...
const EMAIL_PROVIDER = nconf.get("EMAIL_PROVIDER");
...
if (EMAIL_PROVIDER === "SES") {
    ...
    logger.info(`Creating SMTP transport using username: ${nconf.get("smtpConfig").username}`);

    // create SMTP provider
    provider = nodemailer.createTransport({
        host: nconf.get("smtpConfig").server,
        port: nconf.get("smtpConfig").port,
        secure: true,
        requireTLS: true,
        auth: {
            user: nconf.get("smtpConfig").username,
            pass: nconf.get("smtpConfig").password,
        },
    });
 ...
 */
async function sendAccountVerificationEmail(email, code, role) {
    if (!ENABLE_EMAILS) {
        logger.info(`Emails are disabled. Skipping sending verification email to ${email}`);
        return Promise.resolve();
    }

    logger.info(`Sending verification email to ${email}`);

    const oemName = nconf.get("OEM");
    const subject = `${oemName === "skoda" ? "Skoda" : "Allgo"} App Store - ${role === "user" ? "Email Address Verification Request" : "Account Sign up"}`;
    const htmlBody = accountVerificationTemplate(code, role, oemName);
...
async function sendEmailSESWithAttachment(destination, subject, htmlBody) {
    ...
    const data = fs.readFileSync(`${replace(__dirname, /\\/g, "/")}/allgo-logo.png`);
    ...
    attachmentEntity.header("Content-Disposition", "attachment ;filename='allgo-logo.png'");
    attachmentEntity.header("Content-ID", "logo");
    mailContent.body.push(attachmentEntity);

    return ses.sendRawEmail({ RawMessage: { Data: mailContent.toString() } }).promise();
}
...
async function sendPasswordResetEmail(email, url, isPortalUser, username) {
    if (!ENABLE_EMAILS) {
        logger.info(`Emails are disabled. Skipping sending password reset email to ${email}`);
        return Promise.resolve();
    }
    ...
    if (EMAIL_PROVIDER === "SES") {
        ...
        if (isPortalUser) {
            return sendEmailSESWithAttachment(email, subject, htmlBodyPortal, textBody);
        }
        return sendEmailSESWithAttachment(email, subject, htmlBodyCustomer, textBody);
    }
    // send email using SMTP
...
async function sendSubmitFeedbackLinkEmail(email, url) {
    if (!ENABLE_EMAILS) {
        logger.info(`Emails are disabled. Skipping sending submit feedback email to ${email}`);
        return Promise.resolve();
    }
