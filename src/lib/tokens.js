 ...
 * @returns {Promise<String>}
 */
async function createTokenV2(user, sso, accessTokenTtl, type = "access") {
    ...
    const data = {
        ...
        scopes: user.scopes,
        sso_login: !!sso,
        // emailOpted: account.email_opt_in ?? false,
    };

    return jwt.sign(data, nconf.get("jwtConfig").accessTokenSecret, {
...
async function createRefreshToken(userId, ttl) {
    ...
    return jwt.sign(value, nconf.get("jwtConfig").refreshTokenSecret, {
        expiresIn: +ttl,
        subject: userId,
        issuer: nconf.get("JWT_ISSUER"),
    });
    ...
    return async (req, res, next) => {
        ...
        try {
            ...
            if (!req.headers.authorization) {
                throw new HTTPError(401, "No authorization header", ERROR_CODES.MISSING_DATA);
            }
            ...
            if (
                ...
                if (!account || account?.is_deleted) {
                    throw new HTTPError(403, "Account does not exist", ERROR_CODES.NOT_FOUND);
                }
            ...
            res.status(err.statusCode || 403).json(
                new ErrorResponseNew(err, err.errorCode, req?.apiId)
            );
            return res.end();
        }
    };
